//DeliveryStaffPage
import { Col, Container, Row, ButtonGroup, Button, Form } from "react-bootstrap";
...
import { deliveryRows as initialDeliveryRows, updateRows as initialUpdateRows } from './FakeDataDeliver';  // Importing the fake data
...
export default function DeliveryStaffPage() {
    ...
    const updateColumns = [
        ...
        { id: 'phone', label: 'Số điện thoại', minWidth: 150 },
        {
            id: 'deliveryStaffNote', label: 'Ghi chú NVGH', minWidth: 150, render: (value, rowIndex) => (
                <Form.Control
                    as="textarea"
                    rows={3}
                    value={value}
        ...
        {
            id: 'upload', label: 'Upload Image', minWidth: 150, render: () => (
                <Upload {...uploadProps}>
        ...
        {
            id: 'status', label: 'Tình trạng', minWidth: 100, render: (value, rowIndex) => (
                <Select
                    defaultValue={value}
                    style={{ width: 200 }}
        ...
        {
            id: 'update-status', label: 'Cập nhật', minWidth: 100, render: (value, rowIndex) => (
                <Select
                    defaultValue={value}
                    style={{ width: 200 }}
                    onChange={(newValue) => handleStatusChange(newValue, rowIndex)}
    ...
    const handleStatusChange = (newValue, rowIndex) => {
        const newData = selectedTable === 'deliver' ? [...deliveryData] : [...updateData];
        newData[rowIndex].status = newValue;
        selectedTable === 'deliver' ? setDeliveryData(newData) : setUpdateData(newData);
    ...
    const handleSort = (columnId) => {
        ...
        const sortedData = [...rows].sort((a, b) => {
            if (columnId === 'shipmentId' || columnId === 'orderId') {
                ...
                return direction === 'asc' ? a[columnId].localeCompare(b[columnId]) : b[columnId].localeCompare(a[columnId]);
    ...
    return (
        <div>
            <Container className="table-deliver">
                ...
                <h1 className="text-center mb-4">{selectedTable === 'deliver' ? 'ĐƠN HÀNG CẦN GIAO' : 'CẬP NHẬT ĐƠN HÀNG'}</h1>
                ...
                <Button variant="success" className="mt-3" onClick={handleSave}>Lưu</Button>
            </Container>
        </div>
    );
}